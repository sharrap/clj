...
(defun defstate-rhs (name ch mem arg)
  (cond ((eql (car arg) 'record)
         (if (cdr arg)
             `(curry (function ,name) (cons ,(cadr arg) ,mem))
             `(curry (function ,name) (cons ,ch ,mem))))
        ((eql (car arg) 'emit)
         (if mem
...
(defun defstate-args (name ch mem args)
  (if (not args)
      NIL
      (let* ((arg (car args))
            (arg1 (car arg)))
        (list* (cond ((eql arg1 'is)
                      `((eql ,ch ,(cadr arg))
               ...
               (defstate-args name ch mem (cdr args))))))

(defmacro defstate (name ch mem &rest args)
  `(defun ,name ,(if mem `(,mem ,ch) `(,ch))
    ,(list* 'cond (defstate-args name ch mem args))))
...
(defun emit-oper (str)
  (make-instance
   (case (concatenate 'string (reverse str))
    ("<" 'LT)
    ("<=" 'LEQ)
    (">" 'GT)
    (">=" 'GEQ)
...
(defstate operator-state ch str
  ...
  (ismem #\+ "+" record)
  (ismem #\- "-" record)
  (isf #\= (lambda (s) (find s +eqopers+)) record)
